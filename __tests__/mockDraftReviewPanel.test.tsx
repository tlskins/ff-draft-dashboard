...
describe("completed draft scorecard panel", () => {
  ...
  it("opens a dense scorecard and recalculates a position path", () => {
    ...
    expect(screen.getByText("Per-player PAR")).toBeTruthy()
    expect(screen.getAllByRole("columnheader", {name: "Replacement"}).length)
      .toBeGreaterThan(0)
    ...
    expect(screen.getAllByText(/Latest safe:/).length).toBeGreaterThan(0)
    fireEvent.click(screen.getByRole("button", {name: "Alternate paths"}))
    expect(screen.getByText("PAR path 1")).toBeTruthy()
