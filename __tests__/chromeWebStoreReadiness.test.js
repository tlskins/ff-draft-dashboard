...
describe("Chrome Web Store production boundary", () => {
  ...
  it("ships a complete, consistent privacy and listing packet", () => {
    ...
    const support = readFileSync(join(root, "pages", "extension-support.tsx"), "utf8")
    const helpCatalog = readFileSync(join(root, "behavior", "help", "draftyHelp.ts"), "utf8")
    expect(support).toContain('href="/extension-privacy"')
    expect(helpCatalog).toContain("DRAFTY_EXTENSION_STORE_URL")
    expect(support).toContain("Version {DRAFTY_EXTENSION_VERSION} is approved")
